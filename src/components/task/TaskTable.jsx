...
const TaskTable = () => {
  ...
  return (
    <div className="overflow-x-auto p-4">
        ...
        <tbody>
          {tasks.map((task) => (
            <tr key={task.id} className="border-t border-gray-100">
              <td className="py-4 px-4 cursor-pointer text-black-600" onClick={() => openTaskDetails(task)}>
                {task.title}
              </td>
              <td className="py-4 px-4 text-center">
                <div className="flex justify-center">
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${statusColors[task.status] || 'bg-gray-200 text-gray-700'}`}>
      ...
      <AddTaskSidebar open={isAddSidebarOpen} onClose={() => setIsAddSidebarOpen(false)} onSave={addTask} />
      <DetailTaskSidebar open={isDetailSidebarOpen} onClose={() => setIsDetailSidebarOpen(false)} task={selectedTask} />
    </div>
  );
};
