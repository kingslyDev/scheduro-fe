...
const TaskCard = React.memo(({ card, status, isActive, onTaskUpdate }) => {
  ...
  const handleOpenDialog = () => {
    ...
    if (!isDragging) {
      console.log("Setting editedTask and opening dialog:", card); // Debugging
      setEditedTask({ ...card });
  ...
  const handleDeleteTask = () => {
    try {
      if (!card?.id) throw new Error("Task ID is missing");
  ...
  const { priority, deadline, subtasks } = card || {};
